...
local function BMEA_fake_script() -- JojoHub.Handler 
	...
	library:CreateButton(tabFrame,'Gun Mod',false,'GunMod')
	...
	local gunmod = openedCombat.Scroll.GunMod
	...
	function enableTracers() --For all esps
		for _,v in pairs(game.Players:GetPlayers()) do
			...
			bindToRender('tracersRemove',1,function()
				if not _G.tracers then
					Line:Remove()
					RService:UnbindFromRenderStep('tracersRemove')
					RService:UnbindFromRenderStep('tracersMove')
				end
			end)
			if _G.tracers then
				if v.Character and v.Character:FindFirstChild('HumanoidRootPart') ~= nil and v ~= localplr then
					local vector1,onScreen = camera:WorldToViewportPoint(v.Character.HumanoidRootPart.Position)
					...
					bindToRender('tracersMove',1,function()
			...
			v.CharacterAdded:Connect(function()
				...
				if _G.tracers and v ~= localplr then
					Line.Color = v.TeamColor.Color
					Line.Thickness = 2
					Line.From = Vector2.new(camera.ViewportSize.X/2,camera.ViewportSize.Y/2)
					Line.To = Vector2.new(vector1.X,vector1.Y)
			...
			game.Players.PlayerAdded:Connect(function(newplayer)
				...
				repeat wait() until newplayer.Character:FindFirstChild('HumanoidRootPart') ~= nil
				local vector1,onScreen = camera:WorldToViewportPoint(v.Character.HumanoidRootPart.Position)
				local Line = Drawing.new('Line')
				bindToRender('tracersRemove',1,function()
					if not _G.tracers then
				...
				if _G.tracers and newplayer ~= localplr then
					Line.Color = newplayer.TeamColor.Color
					Line.Thickness = 3
					Line.From = Vector2.new(camera.ViewportSize.X/2,camera.ViewportSize.Y/2)
					Line.To = Vector2.new(vector1.X,vector1.Y)
					Line.Visible = true
					...
					bindToRender('tracersMove',1,function()
						if newplayer.Character and newplayer.Character:FindFirstChild('HumanoidRootPart') ~= nil then
							local vector,onScreen = camera:WorldToViewportPoint(v.Character.HumanoidRootPart.Position)
							if onScreen then
								Line.To = Vector2.new(vector.X,vector.Y)
								Line.Visible = true
	...
	function enableEsp()
		...
		for i, v in pairs(game.Players:GetChildren()) do
			...
			function ESP()
				local connection
				connection = bindToRender('esp',1,function()
					if _G.esp then
						if v.Character ~= nil and v.Character:FindFirstChild("HumanoidRootPart") ~= nil and v.Name ~= plr.Name  and v.Character.Humanoid.Health > 0 then 
							...
							if OnScreen then
								...
								Top.Color = v.TeamColor.Color
								Left.Color = v.TeamColor.Color
								Bottom.Color = v.TeamColor.Color
								Right.Color = v.TeamColor.Color
					...
					if _G.esp then
						if newplr.Character ~= nil and newplr.Character:FindFirstChild("HumanoidRootPart") ~= nil and newplr.Name ~= plr.Name  and newplr.Character.Humanoid.Health > 0 then
							...
							if OnScreen then
								...
								Top.Color = newplr.TeamColor.Color
								Left.Color = newplr.TeamColor.Color
								Bottom.Color = newplr.TeamColor.Color
								Right.Color = newplr.TeamColor.Color
	...
	bindToRender('gunsarecool',3,function()
		getfenv(Functions.usethatgun).currentspread = 0 -- NoSpread

		getfenv(Functions.usethatgun).recoil = 0 -- NoRecoil

		if GetLocalWeapon() ~= "none" and GetLocalWeapon():FindFirstChild("Ammo") and _G.gunmod then -- Inf Ammo
			debug.setupvalue(Functions["updateInventory"], 3, GetLocalWeapon():FindFirstChild("Ammo").Value)
		end

		if _G.gunmod and game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("Spawned") and game:GetService("UserInputService"):IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then -- RapidFire
	...
	bindToRender('tweens',1,function()
		...
		if _G.gunmod then
			local tween = ts:Create(gunmod,tweenInfo,{BackgroundColor3 = Color3.fromRGB(0, 255, 0)})
			local tween2 = ts:Create(gunmod.Texting,tweenInfo,{TextColor3 = Color3.fromRGB(0,0,0)})
			tween:Play()
			tween2:Play()
			wait(0.5)
		else
			local tween = ts:Create(gunmod,tweenInfo,{BackgroundColor3 = Color3.fromRGB(255, 0, 0)})
			local tween2 = ts:Create(gunmod.Texting,tweenInfo,{TextColor3 = Color3.fromRGB(145, 145, 145)})
			tween:Play()
			tween2:Play()
			wait(0.5)
		end;
			...
			if pcall(function() readfile(game.PlaceId.."JojohubDrawing.jojo") end) then
				if readfile(game.PlaceId.."JojohubDrawing.jojo") ~= nil then
					local success, response = pcall(function()
						...
						if json.GunMod ~= nil then _G.gunmod = json.GunMod end
